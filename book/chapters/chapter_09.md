...
While attention heads route information, MLP layers transform it. Each MLP neuron can be viewed as a gated feature detector: it activates when the input aligns with a specific direction, and writes a specific output direction. Understanding MLPs is essential for complete circuit reverse engineering.

## Formal Definition

A standard MLP sublayer:
...
Each hidden activation $h_i$ gates an output direction $w_i^{\text{out}}$. This provides a natural connection between MLP neurons and feature-like computations.

### Key Dimensions
- $x$: $[\text{batch}, \text{seq}, d_{\text{model}}]$
- $W_{\text{in}}$: $[d_{\text{model}}, d_{\text{mlp}}]$
- $h$: $[\text{batch}, \text{seq}, d_{\text{mlp}}]$
- $W_{\text{out}}$: $[d_{\text{mlp}}, d_{\text{model}}]$
