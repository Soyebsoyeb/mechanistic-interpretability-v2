...
1. **Attention self-weight**: Let $A = \text{softmax}(QK^\top / \sqrt{d_k}) \in \mathbb{R}^{n \times n}$ be the (row-stochastic) attention matrix for a single head on a sequence of length $n$, so each row sums to 1: $\sum_{j=1}^n A_{ij} = 1$ for all $i$. Its trace,
   $$
   \text{tr}(A) = \sum_{i=1}^n A_{ii},
   $$
   sums the diagonal entries, i.e. the weight each token places on *itself*. Since every entry satisfies $0 \le A_{ij} \le 1$, each diagonal term satisfies $0 \le A_{ii} \le 1$, and so $0 \le \text{tr}(A) \le n$. A trace close to $n$ means every $A_{ii}$ is close to $1$, so the head is close to the identity map (each token mostly attends to itself, and little information is mixed across positions); a trace close to $0$ means every $A_{ii}$ is close to $0$, so the head routes information almost entirely to *other* positions. This is a statement about $A$ *after* the softmax — the raw pre-softmax scores $QK^\top/\sqrt{d_k}$ are not row-stochastic (rows need not sum to 1, and entries can be negative), so their trace has no comparably clean interpretation. Note also that in a causally masked decoder, $A_{ii}$ is still well-defined (attending to one's own position is never masked), so the bound $0 \le \text{tr}(A) \le n$ holds unchanged.
