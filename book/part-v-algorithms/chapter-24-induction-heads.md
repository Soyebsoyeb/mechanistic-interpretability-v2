...
## Motivation
Induction heads copy information from earlier in a sequence. They are a canonical transformer circuit.

## Formalization
Given $[A][B]...[A]$, predict $[B]$. The head attends from second $[A]$ to $[B]$ and copies its information.

## Procedure
1. Construct prompts: $[A][B]...[A]$
...
## Falsification
Falsified if ablating head does not reduce induction score, or if QK lacks [A]->[B] pattern.
